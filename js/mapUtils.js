...
import { currentLocationCoords, currentCenter } from './location.js';
...
export let clickedLocationMarker = null;
    ...
    async initMap(lat, lon) {
        ...
        try {
            ...
            this.map.on('click', async (e) => {
                ...
                currentCenter = { lat, lon: lng };

                if (currentLocationMarker) currentLocationMarker.remove();
                if (searchedLocationMarker) searchedLocationMarker.remove();
                ...
                el.style.backgroundImage = 'url("data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 36" fill="none">
                        <path d="M12 0C5.373 0 0 5.373 0 12c0 8.013 10.432 21.666 11.15 22.62.37.5.92.5 1.29 0C13.568 33.666 24 20.013 24 12 24 5.373 18.627 0 12 0z" fill="#00FF00"/>
                ...
                clickedLocationMarker = new mapboxgl.Marker({ element: el })
                    .setLngLat([lng, lat])
                    .addTo(this.map);

                this.map.flyTo({ center: [lng, lat], zoom: 14 });

                let address = "Clicked Location";
                try {
                    ...
                    console.error("Error reverse geocoding clicked location:", error.message);
                }
                ...
                try {
                    const chargers = await getChargers(lat, lng, distance, fastOnly);
                    addChargersToMap(chargers, [lng, lat], parseFloat(distance));
                    addCircleToMap([lng, lat], parseFloat(distance));
                } catch (error) {
                    console.error("Error fetching chargers for clicked location:", error.message);
                    alert("Error fetching chargers for clicked location: " + error.message);
                }
            });
        } catch (error) {
    ...
    onAdd(map) {
        ...
        this._container.onclick = async () => {
            try {
                ...
                const map = await getMap().catch(error => {
